	...
	render() {
		...
		return (
			<View style={{flex:1}}>
				...
				{ Platform.OS === 'ios' ?
					this.getIOSView() : this.getAndroidView()}
			</View>
		);
	}
	getIOSView () {
		...
		return(
			<ScrollView style={[commonStyles.container, commonStyles.content]} testID="Login" keyboardShouldPersistTaps="handled">
				...
				<View style={{ padding : 20, top : 20}}>
					...
					<TouchableOpacity style ={{marginTop : 10, backgroundColor:"#a9d5d1", width:'100%', height:40, alignItems: 'center', justifyContent:'center', borderRadius:5}}
						onPress={()=> this.onSkip()}>
