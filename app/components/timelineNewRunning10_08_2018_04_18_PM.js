...
export default class TimelineNew extends Component{
        ...
        return (          
          <View style={styles.container}>
            ...
            <View>
              ...
              { this.props.arrAdvertise[this.state.indexOfAdvertise].ad_type === '1' ? 
                  <ImagePlayer uri = {this.props.arrAdvertise[this.state.indexOfAdvertise].path} callback={()=>this.startCounter()} callbackError={this.startCounterWithError.bind(this)}/> 
                  : 
                  this.state.isDownloadStatus === true && this.state.isDownloading === false ?  <VideoPlayer uri = {this.state.videoUrl} /* {this.props.arrAdvertise[this.state.indexOfAdvertise].path}*/ callback={()=>this.startCounter()} callbackError={this.startCounterWithError.bind(this)} onEndPlay={this.nextItemToPlay.bind(this)}/> : this.checkVideoStatus()}
              </View>
              </TouchableWithoutFeedback>
    ...
    checkVideoStatus() {
      ...
      console.log("VideoPlayer componentDidMount:= this.props.uri", this.props.uri)
      console.log("VideoPlayer componentDidMount:=this.state.videoUrl = ", this.state.videoUrl)
 
      if (this.state.isDownloading === false && this.state.videoUrl !== this.props.arrAdvertise[this.state.indexOfAdvertise].path) {
       this.setState({
         isDownloading: true,
         isDownloadStatus: false
       })
       var strUrl = this.props.uri
       var arrFile = strUrl.split("/")
       var fileName = arrFile[arrFile.length-1]
       console.log("Downloadable fileName:=",fileName)
       let dirs = RNFetchBlob.fs.dirs
...
class VideoPlayer extends React.Component {
  constructor(props) {
   ...
   componentDidMount() {
     ...
     if (this.state.isDownloading === false || this.state.videoUrl !== this.props.uri) {
      this.setState({
        isDownloading: true,
        isDownloadStatus: false
      })
   ...
   render() {
     ...
     console.log("this.state.isDownloadStatus:=", this.state.isDownloadStatus)
     console.log("this.state.isDownloading:=", this.state.isDownloading)
     console.log("this.state.videoUrl:=", this.state.videoUrl)
     return (
      <View style={{alignItems: "center", height: height, width: width }}>
        {/* {this.state.isDownloadStatus === true && this.state.isDownloading === false ?  */}
